      ...
      if ((data = hash[key])) {
        if (rel.kind === 'belongsTo') {
          if (Ember.typeOf(hash[key]) === 'object') {
            self.debug(fmt('found 1 embedded %@ record:', rel.type.modelName), hash[key]);
            delete hash[key];
            serializer = store.serializerFor(rel.type.modelName);
          ...
          hash[key] = data.map(function (item) {
            if (Ember.typeOf(item) === 'object') {
              self.debug(fmt('found 1 embedded %@ record:', rel.type.modelName), item);
